CONTAINER_NAME="prg-data-tardis"
...
if $INSTALL_DATA_TARDIS_CONTAINER; then
    ...
    docker run -d --restart=unless-stopped --name="$CONTAINER_NAME" -p 1963:1963 --privileged -v /dev:/dev --env ETCO_synology_password="9^jvH%5cU6#E" --env HOST_HOSTNAME="`hostname`" --env ETCO_delete_after_upload=1 --mount type=bind,source=/media/prg,target=/media/prg,bind-propagation=shared "$IMAGE_NAME"
fi
